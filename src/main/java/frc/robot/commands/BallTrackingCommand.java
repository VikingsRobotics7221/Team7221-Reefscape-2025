 ...
 * 
 * NOW GYRO-FREE! Uses time-based return instead!
 * 
 * coded by paysean
 */
public class BallTrackingCommand extends Command {
    ...
    // ===== RETURNING VARIABLES =====
    private double m_returnDirection = -1.0; // -1 = left turn, 1 = right turn
    private int m_returnPhase = 0; // 0 = backing up, 1 = turning, 2 = driving forward
    ...
    public void initialize() {
        ...
        // Choose a return direction randomly
        m_returnDirection = (Math.random() > 0.5) ? -1.0 : 1.0;
        
        System.out.println("            ____________");
        System.out.println("           /            \\");
        System.out.println("          /  BALL HUNT  \\");
    ...
    * MODIFIED: Now uses time-based strategy instead of gyro!
    */
   private void executeReturning() {
       ...
       // 1. Back up a little
       // 2. Turn around 180°
       // 3. Drive forward to where we started
       
       if (m_returnPhase == 0) {
           // Phase 0: Back up for 1 second
           if (timeInState < 1000) {
               m_drive.arcadeDrive(-0.3, 0);
           } else {
               m_drive.arcadeDrive(0, 0);
               m_returnPhase = 1;
               System.out.println(">> BACK-UP COMPLETE! TURNING AROUND...");
           }
       } 
       else if (m_returnPhase == 1) {
           // Phase 1: Turn around (approximately 180°)
           // A 180° turn at 0.5 power takes about 2 seconds
           if (timeInState < 3000) {
               m_drive.arcadeDrive(0, 0.5 * m_returnDirection);
           } else {
               m_drive.arcadeDrive(0, 0);
               m_returnPhase = 2;
               System.out.println(">> TURN COMPLETE! RETURNING TO START...");
           }
       }
       else if (m_returnPhase == 2) {
           // Phase 2: Drive forward again
           if (timeInState < 5000) {
   ...
   private void changeState(TrackingState newState) {
       ...
       // Reset return phase if entering RETURNING state
       if (newState == TrackingState.RETURNING) {
           m_returnPhase = 0;
       }
